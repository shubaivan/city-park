...
- **Reading the list needs no confirmed account.** Anyone who opens the bot sees the listings, linked to an особовий рахунок or not — a listing is an advertisement, and hiding it from an unlinked newcomer only costs the owner the reader most likely to be flat-hunting. *Publishing* still requires a confirmed account (apartment/address/area are read from it), and `RentalPublish::askRooms` explains that on click rather than the menu hiding the button. When such a person uses the relay, the owner sees `(не підтверджений ОСББ)` instead of an apartment number, so they judge for themselves.
