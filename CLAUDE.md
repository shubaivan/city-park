...
**Slash menu must be pushed via `bin/console bot:menu:update --env=prod` after editing `BotMenuUpdateCommand::MENU`.** It is registered for **private chats only** (`scope: all_private_chats`), and the command also clears the default scope — registered without a scope the commands show in the residents' group too, where a tap does nothing because the global middleware drops group updates. Setting the private scope does not empty the default one; both calls are needed. Its array order is what Telegram renders. `/start` «🏠 Головне меню» leads it (10.09.2026): every other entry opens one section, this one opens everything and is the way back when somebody is lost inside a card — which is exactly when they scroll this list — and fifth among thirteen identical rows it was not being found. 🔑 Оренда keeps the top of the inline menu in `StartCommand::mainMenuMarkup()`, where there is no «головне меню» to compete with it (residents were not finding Оренда at the bottom); the rest of the two orders stay in step. Nutgram's `setMyCommands()` has a null-scope bug; the command uses raw `sendRequest()` instead.
