...
`PavilionPhoto` (artifact) and `PhotoUploadRequest` (obligation) are separate. The cron `pavilion:photo:check` (every 20 min) materialises requests for past sessions inside `PavilionPhotoService::LOOKBACK_HOURS` (26). Reminders fire at end+5/+15 min. Block is gentle (since 2026-06-12): it fires at `PavilionPhotoService::BLOCK_HOUR` (09:00) the **next morning** if the photo is still missing — the user gets the whole booking day + evening to upload (was session-end +20 min before). Reminders that would land 23:00–09:00 Kyiv are deferred to 09:00. After block, the user still has `UPLOAD_GRACE_AFTER_BLOCK_MIN` (120 min / 2h, counted from the actual deferred block instant) to self-upload and auto-unblock; past that the bot refuses the photo and points to the accountant. User-facing copy renders the window via `PavilionPhotoService::uploadGraceLabel()` (the `/info` FAQ string hardcodes "2 години" since it lives in a `const` array).
