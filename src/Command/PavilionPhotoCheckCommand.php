...
class PavilionPhotoCheckCommand extends Command
{
    ...
    private function blockAccount(PhotoUploadRequest $req, \DateTime $now): void
    {
        ...
        $text = sprintf(
            ...
            . "Після цього — лише через Аліну Бухгалтера (+380 93 658 32 02) або голову ОСББ Люду (+380 67 470 46 24).",
            UkDateFormatter::dayDate($start),
            UkDateFormatter::time($start),
            $req->getPavilion(),
    ...
    private function sendGraceWarning(PhotoUploadRequest $req): bool
    {
        ...
        $text = sprintf(
            ...
            . "⛔ Після цього самостійне завантаження буде вимкнено, і розблокувати акаунт можна буде лише через Аліну Бухгалтера (+380 93 658 32 02) або голову ОСББ Люду (+380 67 470 46 24).",
            UkDateFormatter::dayDate($start),
            UkDateFormatter::time($start),
            $req->getPavilion(),
            $cutoff->format('H:i'),
